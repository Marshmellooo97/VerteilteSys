...
int setupConnectionfirst(int _port) {
	int sockfd, connfd;
...
int setupConnectionsecond(int sockfd) {
...
int firstTouch(int sockfd, int _port) {
    ...
    connfd = setupConnectionsecond(sockfd);
	char buffer[MESG_SIZE];
	bzero(buffer, MESG_SIZE);
...
int main(int argc, char** argv) {
    ...
    int sockfd = setupConnectionfirst(port);
	int sockfd2 = setupConnectionfirst(port + 1);
	...
	int t = 0;
   	while(1) {
		firstTouch(sockfd, port);
		int connfd = permantConnection(sockfd2);
